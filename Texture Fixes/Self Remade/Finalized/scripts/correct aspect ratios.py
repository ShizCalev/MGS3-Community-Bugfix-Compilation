...
def pad_image_to_size_centered(
    ...
    if is_opaque:
        alpha_value = opaque_alpha if opaque_alpha is not None else 255

        # black fill, matching alpha (128 or 255)
        result = Image.new("RGBA", (new_width, new_height), (0, 0, 0, alpha_value))
        result.paste(source_image, (offset_x, offset_y))

        return result, f"black padding (alpha={alpha_value})"
