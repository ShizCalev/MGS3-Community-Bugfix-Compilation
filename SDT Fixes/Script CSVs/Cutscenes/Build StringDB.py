...
def resolve_language(lang_tag: str, text: str, csv_path: Path, row_number: int) -> int:
    if JAPANESE_TEXT_PATTERN.search(text):
        return 7

    if lang_tag == "[JPN]":
        return 1 if re.search(r"[A-Za-z]", text) else -1
...
def resolve_final_language(languages: set[int], text: str) -> int:
    if JAPANESE_TEXT_PATTERN.search(text):
        return 7

    definite_languages = {lang for lang in languages if lang != -1}

    if len(definite_languages) == 1:
        return next(iter(definite_languages))
...
def build_string_db(root: Path, csv_files: list[Path]) -> ET.ElementTree:
    ...
    for entry in strings.values():
        final_lang = resolve_final_language(entry.languages, entry.text)
